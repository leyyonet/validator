...
/**
 * Checks if date is not passed by given condition
 *
 * Conditions
 * - date
 *
 * Relations
 * - {@link InRange} - inverse
 */
export function NotInRange(condition?: SD.Value<TimePresentTuple>): PropertyDecorator;
export function NotInRange(condition?: SD.Value<TimePresentTuple>): ParameterDecorator;
export function NotInRange(opt?: O): ParameterDecorator;
export function NotInRange(opt?: O): ParameterDecorator;
export function NotInRange(v1?: SD.Value<TimePresentTuple> | O): PropertyDecorator | ParameterDecorator {
    return (clazz: object, property?: PropertyKey, index?: number) => {
        ...
        validatorHub.pool.add<C, P, Date>({
            ins, opt, constraint,
            error: '{{field}} must be failed by {{equality}}({{produced}},{{comparison}}), reason: {{reason}}',
...
const deco = decorator.addIdentifier(NotInRange, ['field', 'parameter']);
