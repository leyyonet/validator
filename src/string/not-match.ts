...
/**
 * Data must not be matched by given pattern
 *
 * Conditions
 * - string
 *
 ...
 */
export function NotMatch(pattern?: SD.Value<StringPattern>): PropertyDecorator;
export function NotMatch(pattern?: SD.Value<StringPattern>): ParameterDecorator;
export function NotMatch(opt?: O): ParameterDecorator;
export function NotMatch(opt?: O): ParameterDecorator;
export function NotMatch(v1?: SD.Value<StringPattern> | O): PropertyDecorator | ParameterDecorator {
    return (clazz: object, property?: PropertyKey, index?: number) => {
        ...
        validatorHub.pool.add<C, P, string>({
            ins, opt, constraint,
            error: '{{field}} must not be matched by {{pattern}}',
...
const deco = decorator.addIdentifier(NotMatch, ['field', 'parameter']);
