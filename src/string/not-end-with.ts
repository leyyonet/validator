 ...
 */
export function NotEndWith(needle?: SD.Value<string>): PropertyDecorator;
export function NotEndWith(needle?: SD.Value<string>): ParameterDecorator;
export function NotEndWith(opt?: O): ParameterDecorator;
export function NotEndWith(opt?: O): ParameterDecorator;
export function NotEndWith(v1?: SD.Value<string> | O): PropertyDecorator | ParameterDecorator {
    return (clazz: object, property?: PropertyKey, index?: number) => {
        ...
        validatorHub.pool.add<C, P, string>({
            ins, opt, constraint,
            error: '{{field}} must not end with {{needle}}',
...
const deco = decorator.addIdentifier(NotEndWith, ['field', 'parameter']);
