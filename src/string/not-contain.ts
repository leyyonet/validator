...
/**
 * Data must not contain given needle
 *
 * Conditions
 * - string
 *
 ...
 */
export function NotContain(needle?: SD.Value<string>): PropertyDecorator;
export function NotContain(needle?: SD.Value<string>): ParameterDecorator;
export function NotContain(opt?: O): ParameterDecorator;
export function NotContain(opt?: O): ParameterDecorator;
export function NotContain(v1?: SD.Value<string> | O): PropertyDecorator | ParameterDecorator {
    return (clazz: object, property?: PropertyKey, index?: number) => {
        ...
        validatorHub.pool.add<C, P, string>({
            ins, opt, constraint,
            error: '{{field}} must not contain {{needle}}',
...
const deco = decorator.addIdentifier(NotContain, ['field', 'parameter']);
