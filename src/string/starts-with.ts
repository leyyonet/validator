 ...
 */
export function StartsWith(needle?: SD.Value<string>): PropertyDecorator;
export function StartsWith(needle?: SD.Value<string>): ParameterDecorator;
export function StartsWith(opt?: O): ParameterDecorator;
export function StartsWith(opt?: O): ParameterDecorator;
export function StartsWith(v1?: SD.Value<string> | O): PropertyDecorator | ParameterDecorator {
    return (clazz: object, property?: PropertyKey, index?: number) => {
        ...
        validatorHub.pool.add<C, P, string>({
            ins, opt, constraint,
            error: '{{field}} must start with {{needle}}',
...
const deco = decorator.addIdentifier(StartsWith, ['field', 'parameter']);
