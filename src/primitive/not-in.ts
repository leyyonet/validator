...
/**
 * Data must not be in given seeds
 *
 * Conditions
 * - not-empty
 *
 ...
 * - {@link IsIn} - inverse
 */
export function NotIn(seeds?: SD.Value<any>): PropertyDecorator;
export function NotIn(seeds?: SD.Value<any>): ParameterDecorator;
export function NotIn(opt?: O): ParameterDecorator;
export function NotIn(opt?: O): ParameterDecorator;
export function NotIn(v1?: SD.Value<any> | O): PropertyDecorator | ParameterDecorator {
    return (clazz: object, property?: PropertyKey, index?: number) => {
        ...
        validatorHub.pool.add<C, P>({
            ins, opt, constraint,
            error: '{{field}} must not be in {{seeds}}',
...
const deco = decorator.addIdentifier(NotIn, ['field', 'parameter']);
