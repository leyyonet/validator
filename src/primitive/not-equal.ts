 ...
 * - {@link Equals} - inverse
 */
export function NotEqual(compared?: SD.Value<any>): PropertyDecorator;
export function NotEqual(compared?: SD.Value<any>): ParameterDecorator;
export function NotEqual(opt?: O): ParameterDecorator;
export function NotEqual(opt?: O): ParameterDecorator;
export function NotEqual(v1?: SD.Value<any> | O): PropertyDecorator | ParameterDecorator {
    return (clazz: object, property?: PropertyKey, index?: number) => {
        ...
        validatorHub.pool.add<C, P>({
            ins, opt, constraint,
            error: '{{field}} must not be equal to {{compared}}',
...
const deco = decorator.addIdentifier(NotEqual, ['field', 'parameter']);
