...
/**
 * Checks if array contains every value from the given seeds.
 *
 * Conditions
 * - array
 *
 * Relations
 * - {@link NotContainEvery} - inverse
 * - {@link ContainsAny} - any
 */
export function ContainsEvery(seeds: SD.Value<Array<any>>): PropertyDecorator;
export function ContainsEvery(seeds: SD.Value<Array<any>>): ParameterDecorator;
export function ContainsEvery(opt?: O): PropertyDecorator;
export function ContainsEvery(opt?: O): ParameterDecorator;
export function ContainsEvery(v1: SD.Value<Array<any>> | O): PropertyDecorator | ParameterDecorator {
    return (clazz: object, property: PropertyKey, index?: number) => {
        ...
        validatorHub.pool.add<C, P, Arr>(
            {
                ins, opt, constraint,
                error: '{{field}} must contain every of {{seeds}} values',
...
const deco = decorator.addIdentifier(ContainsEvery, ['field', 'parameter']);
