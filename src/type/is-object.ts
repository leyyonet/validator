 ...
 *
 */
export function IsObject(weak?: SD.Value<boolean>): PropertyDecorator;
export function IsObject(weak?: SD.Value<boolean>): ParameterDecorator;
export function IsObject(opt?: O): ParameterDecorator;
export function IsObject(opt?: O): ParameterDecorator;
export function IsObject(v1?: SD.Value<boolean> | O): PropertyDecorator | ParameterDecorator {
    return (clazz: object, property?: PropertyKey, index?: number) => {
        ...
        validatorHub.pool.add<C, P>({
            ins, opt, constraint,
            error: '{{field}} must be an object, but it is {{type}}',
...
const deco = decorator.addIdentifier(IsObject, ['field', 'parameter']);
