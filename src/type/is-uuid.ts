...
import {SD, uuidType} from "@leyyo/scalar";
...
/**
 * Data must be an uuid
 *
 * Conditions
 * - not-empty
 *
 */
export function IsUuid(weak?: SD.Value<boolean>): PropertyDecorator;
export function IsUuid(weak?: SD.Value<boolean>): ParameterDecorator;
export function IsUuid(opt?: O): ParameterDecorator;
export function IsUuid(opt?: O): ParameterDecorator;
export function IsUuid(v1?: SD.Value<boolean> | O): PropertyDecorator | ParameterDecorator {
...
const deco = decorator.addIdentifier(IsUuid, ['field', 'parameter']);
