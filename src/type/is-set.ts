 ...
 *
 */
export function IsSet(weak?: SD.Value<boolean>): PropertyDecorator;
export function IsSet(weak?: SD.Value<boolean>): ParameterDecorator;
export function IsSet(opt?: O): ParameterDecorator;
export function IsSet(opt?: O): ParameterDecorator;
export function IsSet(v1?: SD.Value<boolean> | O): PropertyDecorator | ParameterDecorator {
    return (clazz: object, property?: PropertyKey, index?: number) => {
        ...
        validatorHub.pool.add<C, P>({
            ins, opt, constraint,
            error: '{{field}} must be a set, but it is {{type}}',
...
const deco = decorator.addIdentifier(IsSet, ['field', 'parameter']);
