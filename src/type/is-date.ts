...
import {dateType, SD} from "@leyyo/scalar";
 ...
 *
 */
export function IsDate(weak?: SD.Value<boolean>): PropertyDecorator;
export function IsDate(weak?: SD.Value<boolean>): ParameterDecorator;
export function IsDate(opt?: O): ParameterDecorator;
export function IsDate(opt?: O): ParameterDecorator;
export function IsDate(v1?: SD.Value<boolean> | O): PropertyDecorator | ParameterDecorator {
...
const deco = decorator.addIdentifier(IsDate, ['field', 'parameter']);
deco.addKeyword('validator', 'ph:field', 'ph:deco', 'ph:current', 'ph-type');

export const IsDatetime = IsDate;
