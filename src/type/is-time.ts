...
import {SD, simpleTimeType} from "@leyyo/scalar";
 ...
 *
 */
export function IsTime(weak?: SD.Value<boolean>): PropertyDecorator;
export function IsTime(weak?: SD.Value<boolean>): ParameterDecorator;
export function IsTime(opt?: O): ParameterDecorator;
export function IsTime(opt?: O): ParameterDecorator;
export function IsTime(v1?: SD.Value<boolean> | O): PropertyDecorator | ParameterDecorator {
    return (clazz: object, property?: PropertyKey, index?: number) => {
        ...
        validatorHub.pool.add<C, P>({
            ins, opt, constraint,
            error: '{{field}} must be a time, but it is {{type}}',
            is: (current) => !is.empty(current),
            validates: (current, ctx) => {

                const {weak} = ctx.constraint;
                if (!typeHelper.isValid(weak,
                    () => simpleTimeType.cast(current),
                    () => simpleTimeType.exact(current))) {
...
const deco = decorator.addIdentifier(IsTime, ['field', 'parameter']);
